...
/*:
 * @plugindesc ショップシーンのステータス表示を改造します。
 * @author Yana (改変 munokura)
 * @target MV
 *
 * @help
 * このプラグインはYana氏が作成した装備&ショップステータス改造プラグイン
 * (EquipAndShopStatusR ver1.09) を改変したものです。
 * ステータス表示の変更をショップ画面のみに限定しました。
 * 
 * 
 * ------------------------------------------------------
 *  プラグインコマンドはありません。
...
(function () {
    ...
    var parameters = PluginManager.parameters('MNKR_EquipAndShopStatusR');
    ...
    Window_EquipStatus.prototype.initialize = function (x, y) {
        ...
        const isScene = SceneManager._scene.constructor.name === 'Scene_Equip';
        if (!isScene) {
            this._pageIndex = 0;
        }
    };
    ...
    Window_EquipStatus.prototype.drawParamName = function (paramId, x, y) {
        const isScene = SceneManager._scene.constructor.name === 'Scene_Equip';
        if (isScene) {
            _Window_EquipStatus_drawParamName.call(this, paramId, x, y);
        } else {
    ...
    Window_EquipStatus.prototype.drawItem = function (paramId, x, y) {
        const isScene = SceneManager._scene.constructor.name === 'Scene_Equip';
        if (isScene) {
            _Window_EquipStatus_drawItem.call(this, paramId, x, y);
        } else {
    ...
    const _Window_EquipStatus_windowHeight = Window_EquipStatus.prototype.windowHeight;
    Window_EquipStatus.prototype.windowHeight = function () {
        const isScene = SceneManager._scene.constructor.name === 'Scene_Equip';
        if (isScene) {
            return _Window_EquipStatus_windowHeight.call(this);
        } else {
            return Graphics.height - 108;
        }
    };

    const _Window_EquipStatus_standardPadding = Window_EquipStatus.prototype.standardPadding;
    Window_EquipStatus.prototype.standardPadding = function () {
        const isScene = SceneManager._scene.constructor.name === 'Scene_Equip';
        if (isScene) {
            return _Window_EquipStatus_standardPadding.call(this);
        } else {
            return 6;
        }
    };

    // Window_EquipStatus 未変更
